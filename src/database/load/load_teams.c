...
static void load_team(FILE *database_file, char *line, size_t len, teams_t *teams)
{
    ...
    // TODO users
    if (getline(&line, &len, database_file) == -1) { return; }
    // users_t *users;

    load_channels(database_file, team_data->channels);
}

void load_teams(FILE *database_file, teams_t *teams)
{
    char *line = NULL;
    size_t len = 0;
    ...
    while (getline(&line, &len, database_file)){
        ...
        load_team(database_file, line, len, teams);
    }
